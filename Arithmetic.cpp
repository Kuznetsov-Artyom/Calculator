...
void Arithmetic::GenerationPostForm() {
  ...
  for (size_t i = 0; i < tokens.GetCount(); ++i) {
    ...
    if (tokens[i].type == TypeToken::VARIABLE ||
        tokens[i].type == TypeToken::CONST) {
    ...
    // If the token is an operation
      ...
      while (!stOperation.empty() &&
             tokens[i].value <= stOperation.top().value) {
        postForm.emplace_back(stOperation.top());
        stOperation.pop();
      }
...
std::string Arithmetic::GetSrcStr() const noexcept {
  return tokens.GetSrcStr();
}
  ...
  for (const auto& token : postForm) {
    ...
    // If the token is an operation
      ...
      if (token.arity == Arity::UNARY) {
        ...
        if (token.name == "+") stVariable.push(right);
        if (token.name == "-") stVariable.push(-right);
