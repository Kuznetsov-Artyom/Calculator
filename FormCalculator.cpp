...
System::Void Calculator::FormCalculator::textBoxSrcStr_TextChanged(
...
System::Void Calculator::FormCalculator::buttonGenTable_Click(
    System::Object ^ sender, System::EventArgs ^ e) {
  try {
    std::string inputStr{};
    ...
    for (size_t i = 0; i < row; ++i)
      dataGridViewTableVar->Rows[i]->Height =
          Convert::ToDouble(dataGridViewTableVar->Height) /
              Convert::ToDouble(row) -
          1;

    // Устанавливаем одинаковую ширину столбцов
    for (size_t i = 0; i < col; ++i)
      dataGridViewTableVar->Columns[i]->Width =
          Convert::ToDouble(dataGridViewTableVar->Width) /
    ...
    for (size_t i = 0; i < row; ++i) {
      ...
      dataGridViewTableVar->Rows[i]->Cells[0]->Value =
          gcnew String(it->first.c_str());
      dataGridViewTableVar->Rows[i]->Cells[0]->ReadOnly = true;
    }

    // Заполняем второй столбец значениями операндов (если константа изменение
    // ячейки недоступно)
    for (size_t i = 0; i < row; ++i) {
      auto it = std::next(record.GetTableVar().begin(), i);

      dataGridViewTableVar->Rows[i]->Cells[1]->Value =
    ...
    MessageBox::Show(gcnew String((std::string("Code error: ") +
                                   std::to_string(ex.GetCodeError()) + '\n' +
                                   std::string(ex.what()))
                                      .c_str()),
...
System::Void Calculator::FormCalculator::buttonCalculate_Click(
    System::Object ^ sender, System::EventArgs ^ e) {
  size_t row = record.GetTableVar().size();
  size_t col = 2;
  ...
  for (size_t i = 0; i < row; ++i) {
    ...
    MarshalString(
        Convert::ToString(dataGridViewTableVar->Rows[i]->Cells[0]->Value),
        name);
    MarshalString(
        Convert::ToString(dataGridViewTableVar->Rows[i]->Cells[1]->Value),
        value);

    // Если константа то пропускаем (т.к. неизменяемый элемент)
    if (IsConst(name)) continue;

    // Если переменная, то проверяем на корректность
    if (IsVariable(name)) {
      if ((value[0] == '-' && IsConst(value.substr(1)) && value.size() > 1) ||
          IsConst(value)) {
        record.SetValVar(name, std::atof(value.c_str()));
      } else {
        MessageBox::Show("Некорректный ввод в таблице переменных.", "Ошибка!",
  ...
  try {
    ...
    MessageBox::Show(gcnew String((std::string("Код ошибки: ") +
                                   std::to_string(ex.GetCodeError()) + '\n' +
                                   std::string(ex.what()))
                                      .c_str()),
  ...
  textBoxResult->Text =
      gcnew String(std::to_string(record.GetResult()).c_str());
}
System::Void Calculator::FormCalculator::buttonClear_Click(
    System::Object ^ sender, System::EventArgs ^ e) {
  clearSrcStr();
  clearPolStr();
  clearTable();
