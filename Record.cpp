...
void Record::DefiningTypes() {
  for (auto& token : strTokens) {
    ...
    if (IsConst(token)) {
      tokens.emplace_back(
          Token(TypeToken::CONST, token, std::atof(token.c_str())));
    }
    ...
    // OPERATION type check
      ...
      int priority =
          (token == "+" || token == "-") ? 1 : ((token == "^") ? 3 : 2);
      tokens.emplace_back(
          Token(TypeToken::OPERATION, token, static_cast<double>(priority)));
    }
    ...
    // If the type is not defined (invalid value)
      ...
      throw ExceptionRecord{CodeError::TYPE_UNDEFINED, "Invalid token entered"};
  }
}
...
void Record::CheckingCorrect() {
  ...
  if (!CheckingParentheses(GetSrcStr()))
    throw ExceptionRecord{CodeError::INVALID_PARENTHESES,
                          "Parentheses are entered incorrectly"};

  bool flagVarConst = false;
  bool flagOperation = false;

  for (size_t i = 0; i < tokens.size(); ++i) {
    if (tokens[i].type == TypeToken::CONST ||
        tokens[i].type == TypeToken::VARIABLE) {
      ...
      // We throw an exception if there is no operand in the beginning.
      // expressions or after the opening parenthesis
      ...
      // We throw an exception if there is no operator between the operands and
      ...
      if (tokens[i].type == TypeToken::CLOSE_BRACKET) {
        ...
        if (tokens[i - 1].type == TypeToken::OPEN_BRACKET)
          throw ExceptionRecord{CodeError::EMPTY_PARENTHESES,
                                "Empty parentheses are introduced"};

        // We throw an exception if there is no operand before the closing
        // parenthesis
...
Record::Record(const Record& other)
    : srcStr(other.srcStr), tokens{other.tokens} {}
